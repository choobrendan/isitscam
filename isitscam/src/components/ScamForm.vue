<template>
  <div class="fixed inset-0 flex justify-center items-center z-50 bg-black bg-opacity-70" v-if="showForm">
    <div class="bg-white rounded-3xl w-3/4">
      <div class="flex flex-col items-start p-8">
        <div class="flex items-center w-full">
          ...
          <svg @click="toggleComponent" class="ml-auto fill-current text-gray-700 w-6 h-6 cursor-pointer"
          ...
          <div class="w-full">
            <div class="flex flex-row gap-2 h-10">
              <div class="flex w-4/5">
                <input class="font-bold w-full bg-gray-100 p-2 rounded" type="text" placeholder="Post Title"
                  v-model="title">
              </div>
              <!-- Click to Upload -->
              <div class="flex items-center justify-center w-1/5">
                <label for="dropzone-file"
                  ...
                  <input id="dropzone-file" type="file" class="hidden" @change="previewImages" multiple />
                  <!-- Show the uploaded images -->
                </label>
              </div>
            </div>

            <textarea class="w-full text-sm h-32 bg-gray-100 p-2 rounded mt-2"
              placeholder="Tell us your isit scam story" v-model="content"></textarea>
          </div>

          <div class="w-full my-2" v-if="images.length > 0" :perPage="1">
            <fwb-carousel :pictures="formattedImages" :slide="true" :animation="false">
              <template #default="{ picture }">
                <div class="relative">
                  <img :src="picture.src" :alt="picture.alt" class="object-cover w-full h-full rounded-lg" />
                  <!-- TODO: Delete button not working/showing -->
                  <button class="absolute top-0 right-0 bg-black rounded-full p-1 z-100"
